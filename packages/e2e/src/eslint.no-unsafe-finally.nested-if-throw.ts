...
export const name = 'eslint.no-unsafe-finally.nested-if-throw'
...
export const test: Test = async ({ Command, FileSystem, Main, Workspace }) => {
  ...
  const text =
    "function run(value) { try { console.log(value) } finally { if (value) { throw new Error('x') } } } try { run(true) } catch (error) { console.log(error) }"
