...
export const name = 'eslint.valid-typeof.reversed-equality'

const expectedDiagnostics = [{ source: 'valid-typeof', type: 'error' }]

export const test: Test = async ({ Command, FileSystem, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const uri = `${tmpDir}/test.js`
  const text = "const value = 1; console.log('callable' === typeof value)"
  await FileSystem.writeFiles([
    {
      content: "export default [{ rules: { 'valid-typeof': 'error' } }]",
