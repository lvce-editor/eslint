...
export const name = 'eslint.valid.buffer-global'
...
export const test: Test = async ({ Command, FileSystem, Main, Workspace }) => {
  ...
  const text = "console.log(Buffer.from('x'))"
  await FileSystem.writeFiles([
    {
      content:
        "export default [{ languageOptions: { globals: { console: 'readonly', Buffer: 'readonly' } }, rules: { 'no-undef': 'error' } }]",
